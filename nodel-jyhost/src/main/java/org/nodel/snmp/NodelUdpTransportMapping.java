/*_############################################################################
  _## 
  _##  SNMP4J 2 - (based on) DefaultUdpTransportMapping.java  
  _## 
...
/**
 * (This is based on the {@code DefaultUdpTransportMapping} class within the snmp4j library but incorporates 
 * some Nodel hooks, incl. UDP counters.) 
 * 
 ...
 * @version 1.9
 */
public class NodelUdpTransportMapping extends UdpTransportMapping {

  private static final LogAdapter logger =
      LogFactory.getLogger(NodelUdpTransportMapping.class);
  ...
  // The following lines are nodel related
  
  private static SharableMeasurementProvider s_counterRecvOps = Diagnostics.shared().registerSharableCounter("UDP.Receives", true);
  private static SharableMeasurementProvider s_counterRecvRate = Diagnostics.shared().registerSharableCounter("UDP.Receive rate", true);
  private static SharableMeasurementProvider s_counterSendOps = Diagnostics.shared().registerSharableCounter("UDP.Sends", true);
  private static SharableMeasurementProvider s_counterSendRate = Diagnostics.shared().registerSharableCounter("UDP.Send rate", true);
   ...
   */
  public NodelUdpTransportMapping() throws IOException {
   ...
   * @since 1.7.3
   */
  public NodelUdpTransportMapping(UdpAddress udpAddress,
   ...
   *    if socket binding fails.
   */
  public NodelUdpTransportMapping(UdpAddress udpAddress) throws IOException {
    super(udpAddress);
    socket = new DatagramSocket(udpAddress.getPort(),
      ...
      throws java.io.IOException
  {
    ...
    s_counterSendOps.incr();
    s_counterSendRate.add(dp.getLength() * 8);
  }
        ...
        try {
          ...
          try {
            ...
            try {
                ...
                s_counterRecvOps.incr();
                s_counterRecvRate.add(packet.getLength() * 8);
            }
          ...
          TransportStateReference stateReference =
            new TransportStateReference(NodelUdpTransportMapping.this, udpAddress, null,
          ...
          synchronized (NodelUdpTransportMapping.this) {
            listener = null;
          }
          logger.error(purex);
          if (logger.isDebugEnabled()) {
