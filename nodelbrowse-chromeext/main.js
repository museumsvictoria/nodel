...
ServiceFinder.prototype.recv_ = function(sock, info) {
  ...
  var fullPacket = String.fromCharCode.apply(null, new Uint8Array(data));
  
  var packetObj = JSON.parse(fullPacket);
  
  var httpAddress = packetObj.addresses[1];
  
  var parser = document.createElement('a');
  parser.href = httpAddress;
  
  var addr = parser.hostname + ":" + parser.port;
  
  var byIP = getDefault_(this.byIP_, addr, {});
  
  packetObj.present.forEach(function(nodeName) {
    var ptr = nodeName;
    var byService = getDefault_(this.byService_, ptr, {});
    byService[addr] = ptr;
    byIP[ptr] = parser.href;
  }.bind(this));
...
ServiceFinder.prototype.broadcast_ = function(sock, address) {
  var packet = '{ "discovery" : [ "*" ], "types" : [ "tcp", "http" ] }';
  ...
  var raw = str2ab(packet);
  
  ServiceFinder.api.sendTo(sock, raw, '224.0.0.252', 5354, function(writeInfo) {
    console.info('bytesWritten=' + raw.byteLength)
  });
};
...
window.addEventListener('load', function() {
  ...
  var callback_ = function(opt_error) {
    ...
    outer.apply(finder).forEach(function(o) {
      ...
      if (mode == 'ip') a.href = 'http://'+o;
      else a.href = finder.byIP_[inner.call(finder, o)[0]][o].replace('%NODE%',o);
      a.innerHTML = getHtml_(outer, o);
      a.target = "_blank";
      li.appendChild(a);
      ...
      inner.call(finder, o).forEach(function(i) {
        var li = document.createElement('li');
        var a = document.createElement('a')
        if (mode == 'service') a.href = 'http://'+i;
        else a.href = finder.byIP_[this.valueOf()][i].replace('%NODE%',i);
        a.innerHTML = getHtml_(inner, i);
        a.target = "_blank";
        li.appendChild(a);
        ul.appendChild(li);
      },o);
